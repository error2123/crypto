# implemented as in here: https://www.thebalance.com/engulfing-candle-day-trading-strategy-1030873
...
def render_plotly_ema(df, chart_name='engulfing_candle'):
...
def compute_engulfing_candles_pnl(df, ema=60):
    indexes = [x for x in df.index][60:]
    trend_slope = 0
    buys = []
    sells = []
    last_high = 0
    last_low =
    for x in xrange(1, len(indexes)):
        trend_slope = df.loc[indexes[x-1]].loc["ema_60"] - df.loc[indexes[x]].loc["ema_60"]
        # if its an up trend
        if trend_slope > 0:
            # if the previous candle was red
            if df.loc[indexes[x-1]].loc["open"] >  df.loc[indexes[x-1]].loc["close"]:
                # if the current candle was green
                if df.loc[indexes[x]].loc["open"] >  df.loc[indexes[x]].loc["close"]:
                    if df.loc[indexes[x-1]].loc["open"] <= df.loc[indexes[x]].loc["close"] \
                            and df.loc[indexes[x-1]].loc["close"] >= df.loc[indexes[x]].loc["open"]:
                            buys.append((indexes[x], df.loc[indexes[x]].loc["close"]))
