...
def compute_buy_sell_from_rsi(rsi_sma, min, max, vub, vlb):
    ...
    for x in xrange(1, len(indexes)):
        ...
        if rsi_sma[indexes[x]] < min and df.loc[indexes[x]].loc['volume'] > vub * df.loc[indexes[x]].loc['moving_avg_vol'] and bought is False:
        ...
        elif rsi_sma[indexes[x]] > max and df.loc[indexes[x]].loc['volume'] < df.loc[indexes[x]].loc['moving_avg_vol']/vlb and bought is True:
...
for x in xrange(1, 10, 1):
    for y in xrange(1, 10, 1):
        rsi_buys, rsi_sells, rsi_val, rsi_pnl = compute_buy_sell_from_rsi(rsi_ewma, lb, ub, x, y)
        if len(rsi_pnl) > 0:
