...
def compute_buy_sell_from_rsi_v2(rsi_sma, min, max):
    """
    Same as compute_buy_sell_from_rsi but sells the stock if price reaches where we bought it(stop loss)
    :param rsi_sma:
    :param min:
    :param max:
    :return:
    ...
    for x in xrange(1, len(indexes)):
        ...
        if bought and buys[-1][0] >= df.loc[indexes[x]].loc['close'] + 1:
            sold_at, sold_time = df.loc[indexes[x]].loc['close'], indexes[x]
            sells.append((sold_at, sold_time))
            bought = False
            rsi_l.append((rsi_sma[indexes[x]], sold_time))
            total_profit += sold_at - bought_at
            print(
            "Bought at {} on {}, Sold at {} on {} Profit/loss {}".format(bought_at, bought_time, sold_at, sold_time,
                                                                         sold_at - bought_at))
            entered_sell_zone = False
...
while(True):
    ...
    print "Profit based on MFI_SMA, 30, 70"
    mfi_buys, mfi_sells, mfi_val = compute_buy_sell_from_rsi(mfi_sma, 30, 70)
    ...
    print "Profit based on MFI_EWMA, 40, 70"
    mfi_buys, mfi_sells, mfi_val = compute_buy_sell_from_rsi(mfi_ewma, 40, 70)

    print "No. of total MFI_EWMA trades {}".format(len(mfi_val))
    # print rsi_ewma, rsi_sma
    render_plotly_ema(df, [mfi_ewma, mfi_sma], [mfi_buys, mfi_sells, mfi_val], "mfi_ema.html")


    print "Profit based on RSI_SMA, 30, 70"
    rsi_buys, rsi_sells, rsi_val = compute_buy_sell_from_rsi(rsi_sma, 30, 70)

    print "No. of total RSI_SMA trades {}".format(len(rsi_val))

    render_plotly_ema(df, [rsi_ewma, rsi_sma], [rsi_buys, rsi_sells, rsi_val], "rsi_sma.html")
    ...
    rsi_buys, rsi_sells, rsi_val = compute_buy_sell_from_rsi(rsi_ewma, 40, 70)

    print "No. of total RSI_EMA trades {}".format(len(rsi_val))
    # print rsi_ewma, rsi_sma
    render_plotly_ema(df, [rsi_ewma, rsi_sma], [rsi_buys, rsi_sells, rsi_val], "rsi_ema.html")

    #print "Profit based on RSI_EMA_WITH_STOP_LOSS"
    #si_buys, rsi_sells, rsi_val = compute_buy_sell_from_rsi_v2(rsi_ewma, 30, 60)


    print "Profit based on RSI_EMA 30, 60"
    rsi_buys, rsi_sells, rsi_val = compute_buy_sell_from_rsi(rsi_ewma, 30, 60)
    print "No. of total RSI_EMA trades {}".format(len(rsi_val))
